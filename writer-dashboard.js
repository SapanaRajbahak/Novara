...
const WRITER_SECTIONS = [
  ...
  },
  {
    key: "earnings",
    label: "Earnings",
    render: renderWriterEarningsSection,
  },
  {
    key: "ai-tools",
    label: "AI Tools",
...
function getVisibleStories() {
  ...
  const filtered = list.filter((story) => {
    ...
    const genre = String(getStoryGenre(story) || "").toLowerCase();
    const description = String(story.description || "").toLowerCase();
    return title.includes(writerStoriesViewState.search) || genre.includes(writerStoriesViewState.search) || description.includes(writerStoriesViewState.search);
  });
  ...
  filtered.sort((a, b) => {
    ...
    const timeA = parseStoryDate(a)?.getTime() || 0;
    const timeB = parseStoryDate(b)?.getTime() || 0;
    return timeB - timeA;
  });
...
function renderWriterStoriesList(stories) {
  const container = document.getElementById("writerStoriesDynamicList");
  if (!container) {
    return;
  }
  ...
  if (!Array.isArray(stories) || stories.length === 0) {
    container.innerHTML = `
      <div class="story-card story-card-empty">
        <div class="story-title">No matching books yet</div>
        <div class="story-meta">Create your first book or change your filters to see books here.</div>
  ...
  container.innerHTML = stories
    .map((story) => {
      ...
      const statusClass = getStoryStatusBadgeClass(story.status);
      const chapters = Number(story.chapters || 0).toLocaleString();
      const reads = Number(story.reads || 0).toLocaleString();
      const favorites = Number(story.favorites || 0).toLocaleString();
      const description = escapeStoryHtml(story.description || "No story description yet. Add a summary so readers know what to expect.");
      const genre = escapeStoryHtml(getStoryGenre(story));
      const coverClass = genreCoverClass(genre);
      const coverUrl = escapeStoryHtml(getStoryCoverUrl(story));
      const dateLabel = formatStoryDate(story);
      const isPublished = normalizeStoryStatus(story.status) === "PUBLISHED";
      return `
        <div class="story-card" data-story-id="${id}">
          ${coverUrl
            ? `<img class="story-cover story-cover-image" src="${coverUrl}" alt="${title} cover" loading="lazy" onerror="this.replaceWith(Object.assign(document.createElement('div'),{className:'${coverClass}',textContent:'${genre}'}));" />`
            : `<div class="${coverClass}">${genre}</div>`}
          <div class="story-main">
            ...
            <div class="story-meta">Last edited: ${dateLabel} <span>Chapters: ${chapters}</span> <span>Reads: ${reads}</span> <span>Favorites: ${favorites}</span></div>
            <div class="story-actions">
              <a class="btn btn-soft" href="writer-story-chapters.html?storyId=${id}">Edit</a>
              <a class="btn btn-soft" href="writer-story-chapters.html?storyId=${id}">Manage Chapters</a>
              <a class="btn btn-soft" href="book.html?id=${id}">Preview</a>
              ${isPublished
                ? `<button class="btn btn-primary" type="button" disabled>Published</button>`
                : `<button class="btn btn-primary" type="button" data-action="publish-story">Publish</button>`}
            </div>
          </div>
        </div>
      `;
...
async function loadWriterStoriesSectionData() {
  ...
  container.innerHTML = `
    <div class="story-card">
      <div class="story-title">Loading your books...</div>
    </div>
  `;

  try {
    const response = await fetch(`${resolveWriterApiBaseUrl()}/api/writer/stories`, {
      cache: "no-store",
      credentials: "include",
    });
    ...
    writerStoriesViewState.stories = Array.isArray(payload.data) ? payload.data : [];
    applyWriterStoriesView();
  } catch (error) {
    container.innerHTML = `
      <div class="story-card">
        <div class="story-title">Could not load your books</div>
        <div class="story-meta">Please refresh or try again in a moment.</div>
      </div>
    `;
  }
}
