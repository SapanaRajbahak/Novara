...
bootstrap();
// Monetization panel open/close and tab logic
...
function openMonetizationTab(tabKey) {
  if (!monetizationPanel) return;
  // Hide all other panels except monetizationPanel
  document.querySelectorAll('.panel').forEach(p => {
    if (p !== monetizationPanel) p.style.display = 'none';
  });
  monetizationPanel.style.display = '';
  monetizationPanel.scrollIntoView({behavior:'smooth'});
  // Set active tab
  ...
  Object.keys(monetizationSections).forEach(key => {
    monetizationSections[key].style.display = (key === tabKey) ? 'block' : 'none';
  });
}
...
if (monetizationTabs && monetizationTabs.length) {
  monetizationTabs.forEach(tab => {
    tab.addEventListener('click', () => {
      // Set active tab
      monetizationTabs.forEach(t => t.classList.remove('active'));
      tab.classList.add('active');
      // Show correct section
      Object.keys(monetizationSections).forEach(key => {
        monetizationSections[key].style.display = (tab.dataset.tab === key) ? 'block' : 'none';
      });
    });
  });
}

// Optional: Hide panel on outside click or Esc
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && monetizationPanel && monetizationPanel.style.display !== 'none') {
    monetizationPanel.style.display = 'none';
  }
});
document.addEventListener('mousedown', (e) => {
  if (monetizationPanel && monetizationPanel.style.display !== 'none' && !monetizationPanel.contains(e.target) && e.target !== openMonetizationPanelBtn) {
    monetizationPanel.style.display = 'none';
  }
});
