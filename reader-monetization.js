const CFG = {
  ...
  MAX_FREEZES: 1,
  COIN_PACKS: [
    { id: 'pack_50', coins: 50, price: 0.99, priceDisplay: '$0.99' },
    { id: 'pack_200', coins: 200, price: 3.99, priceDisplay: '$3.99', featured: true },
    { id: 'pack_500', coins: 500, price: 4.99, priceDisplay: '$4.99' },
  ],
  PLANS: [
    {
      id: 'monthly',
      ...
      per: 'per month',
      coins: 200,
      perks: ['200 coins/month allowance', 'No ads ever', 'Early chapter access', 'Priority support'],
      active: true,
    },
    {
      id: 'yearly',
      ...
      save: 'Save 33%',
      coins: 2400,
      perks: ['2,400 coins/year allowance', 'No ads ever', 'Early chapter access', '1.5× streak rewards'],
      active: false,
    },
  ],
  CHAPTERS: [
...
const state = {
  ...
  activePlanId: CFG.PLANS.find(p => p.active)?.id || null,
  currentCheckinDay: 1,
  claimedCheckinDays: [],
  ...
  history: [
    { type: 'subscription', title: 'Pro Monthly activated', amount: +200, date: new Date(), icon: '⭐' },
    { type: 'spent', title: 'Unlocked Chapter 4 — The Secret', amount: -10, date: new Date(Date.now() - 86400000), icon: '🔓' },
    { type: 'earned', title: 'Daily check-in reward', amount: +3, date: new Date(Date.now() - 2 * 86400000), icon: '📅' },
  ],
};
...
function renderHero() {
  ...
  if (activePlan) {
    ...
    heroRenew.textContent = `${activePlan.coins} coins included`;
...
async function buyPack(packId) {
  ...
  try {
    const res = await fetch('http://localhost:5002/api/billing/create-coin-checkout', {
      method: 'POST',
...
async function subscribePlan(planId) {
  ...
  try {
    const res = await fetch('http://localhost:5002/api/billing/create-subscription-checkout', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
function claimCheckin() {
  if (state.claimedCheckinDays.includes(state.currentCheckinDay)) return;

  const reward = CFG.CHECKIN_REWARDS[state.currentCheckinDay - 1] || 0;
  state.balance += reward;
  state.claimedCheckinDays.push(state.currentCheckinDay);

  addTransaction({
    type: 'earned',
    title: `Daily check-in Day ${state.currentCheckinDay}`,
    amount: reward,
    icon: '📅',
  });

  toast(`Daily check-in claimed: +${reward} coins`, '📅', 'success');

  if (state.currentCheckinDay < 7) {
    state.currentCheckinDay += 1;
  } else {
    state.currentCheckinDay = 1;
    state.claimedCheckinDays = [];
  }
...
function rewardAd(isBonus = false) {
  const reward = isBonus ? CFG.BOOK_AD_BONUS : CFG.AD_REWARD;
  state.balance += reward;
  ...
  addTransaction({
    type: 'earned',
    title: isBonus ? 'Bonus completion ad reward' : 'Watch & Earn reward',
    amount: reward,
    icon: '📺',
  });

  toast(`Ad completed: +${reward} coins`, '📺', 'success');
  renderAll();
}
...
function completeBook() {
  state.balance += CFG.BOOK_REWARD;

  addTransaction({
    type: 'earned',
    title: 'Book completion reward',
    amount: CFG.BOOK_REWARD,
    icon: '🎉',
  });
