...
function getSaveKey() {
  return readerScoped("savedBookIds");
}
...
function reloadLocalSetsFromStorage() {
  const saveKey = getSaveKey();
  const favKey = getFavoriteKey();
  if (!localStorage.getItem(saveKey)) {
    const blob = localStorage.getItem(readerScoped("savedBooks"));
    if (blob) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length) {
          if (typeof parsed[0] === "string") {
            ...
            const ids = parsed.map((e) => (e && typeof e === "object" ? e.bookId : null)).filter(Boolean);
            if (ids.length) {
              localStorage.setItem(saveKey, JSON.stringify([...new Set(ids)]));
            }
          }
        }
      } catch (e) {
  ...
  savedBooks = loadSet(saveKey);
  favoriteBooks = loadSet(favKey);
}
...
function renderRelatedBooks(relatedBooks) {
  ...
  relatedBooks.forEach((book) => {
    ...
    card.innerHTML = `<strong>${book.title}</strong><p>${book.authorName || "Unknown Author"}</p><a class="btn" href="/reader/book.html?id=${encodeURIComponent(book.id)}">Open</a>`;
...
function setupChapterEvents() {
  ...
  elements.trackList.addEventListener("click", (event) => {
    ...
    window.location.href = `/reader/audiobook.html?book=${encodeURIComponent(currentBook.id)}`;
  });
}
...
function setupPrimaryActions() {
  ...
  elements.saveBtn.addEventListener("click", () => {
    ...
    persistSet(getSaveKey(), savedBooks);
  });
