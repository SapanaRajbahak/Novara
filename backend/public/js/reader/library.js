...
function readLocalProgressMap() {
  try {
    const parsed = JSON.parse(localStorage.getItem(readerScoped("progress")) || "{}");
...
function syncHomeLinksForAuth(_isSignedIn) {
  ...
  elements.homeLinks.forEach((link) => {
    link.setAttribute("href", "/reader/reader-dashboard.html");
  });
}
...
async function ensureSignedInUser() {
  try {
    if (window.NovaraSession && typeof window.NovaraSession.fetchCurrentUser === "function") {
      ...
      state.isSignedIn = Boolean(user && user.id);
  ...
  card.innerHTML = `
    ...
    <div>
      ...
      <div class="inline-actions">
        <a class="solid" href="/reader/reader.html?bookId=${encodeURIComponent(item.id)}${chapterIdParam}">Resume</a>
      </div>
    </div>
  `;
  return card;
...
function createBookCard(book) {
  ...
  card.innerHTML = `
    ...
    <div class="inline-actions">
      ...
      <a class="solid" href="/reader/reader.html?bookId=${encodeURIComponent(book.id)}">Read</a>
