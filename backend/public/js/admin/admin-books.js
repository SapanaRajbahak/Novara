...
function requireAuth() {
  ...
  if (auth !== "1") {
    const next = encodeURIComponent("/admin/admin-books.html");
...
function createActions(book) {
  ...
  return `
    <div class="actions">
      ...
      <a href="/reader/book.html?id=${encodeURIComponent(book.id)}" data-action="preview" data-id="${book.id}">Preview</a>
    </div>
  `;
}
...
function hasEditControls() {
  return Boolean(
    elements.editModal
      ...
      && elements.editAuthor
      && elements.editGenre
      && elements.editFileType
      && elements.editStatus
      && elements.editTags
      && elements.editDescription
      && elements.editIsAudiobookAvailable
      && elements.editIsAiGenerated
  );
}

function openEditModal(book) {
  if (!hasEditControls()) {
    showToast("Edit form is not available in this layout yet");
    return;
  }
...
async function handleAction(action, id) {
  ...
  if (action === "chapters") {
    window.location.href = `/admin/admin-chapters.html?bookId=${encodeURIComponent(book.id)}`;
    return;
  }

  if (action === "audio") {
    window.location.href = `/admin/admin-audiobooks.html?bookId=${encodeURIComponent(book.id)}`;
  }
}
...
function bindEvents() {
  ...
  if (elements.viewToggle) {
    elements.viewToggle.addEventListener("click", (event) => {
      ...
      state.view = button.dataset.view === "grid" ? "grid" : "table";
      elements.viewToggle.querySelectorAll("button[data-view]").forEach((item) => {
  ...
  if (elements.resetFilters) {
    elements.resetFilters.addEventListener("click", () => {
      ...
      if (elements.viewMode) {
        elements.viewMode.value = "table";
      }
      if (elements.viewToggle) {
        elements.viewToggle.querySelectorAll("button[data-view]").forEach((item) => {
          const isActive = item.dataset.view === "table";
          item.classList.toggle("active", isActive);
          item.setAttribute("aria-pressed", isActive ? "true" : "false");
        });
      }
  ...
  if (!elements.editBookForm || !hasEditControls()) {
    return;
  }
