...
const elements = {
  ...
  chapterFormHeading: document.getElementById("chapterFormHeading"),
  chapterFormSubcopy: document.getElementById("chapterFormSubcopy"),
  chapterFormSubmitBtn: document.getElementById("chapterFormSubmitBtn"),
...
function updateChapterWordCount() {
  if (!elements.chapterWordCount) {
    return;
  }
  const totalWords = countWords(elements.chapterContent.value);
  elements.chapterWordCount.textContent = `Word count: ${totalWords.toLocaleString()}`;
}
...
function enterEditMode(chapter) {
  writerState.editingChapterId = chapter.id;
  elements.chapterNumber.value = String(chapter.chapterNumber || "");
  elements.chapterTitle.value = chapter.title || "";
  elements.chapterContent.value = chapter.content || "";
  elements.isPublished.checked = Boolean(chapter.isPublished);
  if (elements.chapterFormHeading) elements.chapterFormHeading.textContent = `Edit Chapter ${chapter.chapterNumber}`;
  if (elements.chapterFormSubcopy) elements.chapterFormSubcopy.textContent = "Update the chapter details below, then click Save Changes.";
  if (elements.chapterFormSubmitBtn) elements.chapterFormSubmitBtn.textContent = "Save Changes";
  if (elements.cancelEditBtn) elements.cancelEditBtn.style.display = "";
  elements.chapterForm.scrollIntoView({ behavior: "smooth" });
  elements.chapterTitle.focus();
}

function exitEditMode() {
  writerState.editingChapterId = "";
  resetForm();
  if (elements.chapterFormHeading) elements.chapterFormHeading.textContent = "Add Chapter";
  if (elements.chapterFormSubcopy) elements.chapterFormSubcopy.textContent = "Draft new chapters here, then open the full editor when you want to refine structure and prose.";
  if (elements.chapterFormSubmitBtn) elements.chapterFormSubmitBtn.textContent = "Save Chapter";
...
async function openEditor(chapterId) {
  ...
  try {
    ...
    const errMsg = error.message || "Unable to load chapter details.";
    setEditorMessage(errMsg, "error");
    elements.chapterEditorMeta.textContent = errMsg;
...
function bindChapterForm() {
  ...
  if (elements.wordCountBtn) {
    elements.wordCountBtn.addEventListener("click", () => {
      updateChapterWordCount();
      if (window.NovaraSession && typeof window.NovaraSession.showToast === "function") {
        const totalWords = countWords(elements.chapterContent.value);
        window.NovaraSession.showToast(`Chapter has ${totalWords.toLocaleString()} words.`);
      }
    });
  }

  if (elements.chapterContent) {
    elements.chapterContent.addEventListener("input", updateChapterWordCount);
  }
  ...
  elements.chapterForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (writerState.editingChapterId) {
        ...
        response = await fetch(`${window.NovaraSession.API_BASE_URL}/api/writer/chapters/${writerState.editingChapterId}`, {
          method: "PUT",
          credentials: "include",
        ...
        // Create new chapter
        response = await fetch(`${window.NovaraSession.API_BASE_URL}/api/writer/stories/${storyId}/chapters`, {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
      ...
      const payload = await response.json().catch(() => ({ success: false, error: "Unable to save chapter" }));
      if (!response.ok || !payload.success) {
        showError(payload.error || "Unable to save chapter");
        return;
      }

      const wasEditing = !!writerState.editingChapterId;
      exitEditMode();
      await reloadAll();
      window.NovaraSession.showToast(wasEditing ? "Chapter updated successfully." : "Chapter saved successfully.");
    } catch (error) {
...
function bindChapterActions() {
  elements.chapterList.addEventListener("click", async (event) => {
    ...
    if (button.dataset.action === "edit") {
      try {
        const chapter = await fetchChapterById(chapterId);
        enterEditMode(chapter);
      } catch (err) {
        showError("Unable to load chapter details.");
      }
      return;
    }
    ...
    if (button.dataset.action === "publish") {
      const chapter = writerState.chapters.find((item) => item.id === chapterId);
      ...
      const payload = await response.json().catch(() => ({ success: false }));
      if (!response.ok || !payload.success) {
        showError(payload.error || "Unable to publish chapter right now.");
        return;
      }

      await reloadAll();
      window.NovaraSession.showToast("Chapter published successfully.");
      return;
    }
    ...
    if (button.dataset.action === "unpublish") {
      ...
      const payload = await response.json().catch(() => ({ success: false }));
      if (!response.ok || !payload.success) {
        showError(payload.error || "Unable to unpublish chapter right now.");
        return;
      }

      await reloadAll();
      window.NovaraSession.showToast("Chapter moved to draft.");
      return;
    }
...
async function bootstrap() {
  ...
  if (new URLSearchParams(window.location.search).get("addChapter") === "1" && elements.chapterForm) {
    setTimeout(() => {
      elements.chapterForm.scrollIntoView({ behavior: "smooth" });
      if (elements.chapterTitle) elements.chapterTitle.focus();
    }, 400);
  }
}
