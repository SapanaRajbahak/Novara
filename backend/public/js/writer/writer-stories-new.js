...
function wireCoverInput() {
  elements.coverUpload.addEventListener("change", () => {
    ...
    if (file.size > MAX_COVER_SIZE_BYTES) {
      ...
      elements.coverName.textContent = "Image is too large. Max size is 5 MB.";
      showError("Cover image is too large. Please choose an image smaller than 5 MB.");
      return;
    }
...
function bindForm() {
  elements.storyForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      window.location.href = `/writer/writer-story-chapters.html?storyId=${encodeURIComponent(result.data.id)}`;
    } catch (error) {
