...
async function refreshProfilePage() {
  const [profilePayload, statsPayload, libraryPayload, activityPayload, sessionUser] = await Promise.all([
  ...
  state.stats = statsPayload.data || {};
  state.library = Array.isArray(libraryPayload.data) ? libraryPayload.data : [];
  state.activity = Array.isArray(activityPayload.data) ? activityPayload.data : [];

  renderReaderStats(state.stats.reader || null);
  renderWriterStats(state.stats.writer || null);
